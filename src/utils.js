let isMobile = /iphone|ipad|android|phone|mobile/.test(window.navigator.userAgent.toLocaleLowerCase())
...
let $util = {
  ...
  extend: function (base, input) {
    ...
    let result = $util.clone(base, true)
  ...
  bind: function (func, thisArg) {
    ...
    return function () {
      let inputArgs = Array.prototype.slice.call(arguments, 0)
  ...
  contain: function (host, obj) {
    ...
    let invalid = $util.some(obj, function (value, key) {
      if (value !== host[ key ]) {
        return true
      }
    })
  ...
  removeAll: function (array, reason) {
    ...
    let index = $util.indexOf(array, reason)
    if (index > -1) {
  ...
  find: function (array, reason) {
    let index = this.indexOf(array, reason)
